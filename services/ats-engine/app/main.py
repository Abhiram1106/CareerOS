...
app = FastAPI(title="CareerOS ATS Engine", version="0.1.0")
...
def scan(payload: ScanRequest):
    ...
    format_score = 82
    action_hits = sum(1 for w in ["built", "developed", "designed", "optimized", "deployed", "led"] if w in payload.experience_bullet.lower())
    metric_hits = len(re.findall(r"\b\d+(%|x|k|m)?\b", payload.experience_bullet.lower()))
    quality = min(95, 55 + action_hits * 5 + metric_hits * 6)

    completeness_fields = [payload.full_name, payload.email, payload.target_role, payload.skills_csv, payload.summary, payload.experience_bullet]
