...
def build_cohort_readiness_html(
    *,
    generated_at: str,
    college_label: str,
    students_scored: int,
    ...
    skill_gap_rows: list[dict[str, int | str]],
) -> str:
    template_path = Path(__file__).resolve().parents[1] / "templates" / "cohort_readiness_report.html"
    template = template_path.read_text(encoding="utf-8")

    heatmap_table = _render_table(
        ["Department", "Strong", "Ready", "Borderline", "At risk"],
        [
            [
                row["name"],
                row["strong"],
    ...
    gaps_table = _render_table(
        ["Skill", "Students missing"],
        [[row["skill"], row["student_count"]] for row in skill_gap_rows],
    )

    replacements = {
        "{{GENERATED_AT}}": html.escape(generated_at),
        "{{COLLEGE_LABEL}}": html.escape(college_label),
        "{{STUDENTS_SCORED}}": str(students_scored),
        "{{AVG_READINESS}}": f"{avg_readiness:.1f}",
        "{{PARSE_SAFE_RATE}}": f"{parse_safe_rate:.1f}",
        "{{READY_COUNT}}": str(ready_count),
        "{{BUCKET_STRONG}}": str(buckets.get("strong", 0)),
        "{{BUCKET_READY}}": str(buckets.get("ready", 0)),
        "{{BUCKET_BORDERLINE}}": str(buckets.get("borderline", 0)),
        "{{BUCKET_RISK}}": str(buckets.get("risk", 0)),
        "{{HEATMAP_TABLE}}": heatmap_table,
        "{{SKILL_GAPS_TABLE}}": gaps_table,
    }
    out = template
...
def generate_cohort_readiness_pdf_bytes(html_doc: str) -> bytes:
    if WEASYPRINT_ENABLED:
        try:
            from weasyprint import HTML
            ...
            pass
    title = "CareerOS Cohort Readiness Report"
    snippet = re.sub(r"<[^>]+>", " ", html_doc)[:400]
    payload = (
        "%PDF-1.1\n"
        "1 0 obj<</Type/Catalog/Pages 2 0 R>>endobj\n"
        "2 0 obj<</Type/Pages/Count 1/Kids[3 0 R]>>endobj\n"
        "3 0 obj<</Type/Page/Parent 2 0 R/MediaBox[0 0 612 792]/Contents 4 0 R>>endobj\n"
        f"4 0 obj<</Length {len(snippet) + 60}>>stream\nBT /F1 12 Tf 72 740 Td ({title}) Tj ET\n"
        f"BT /F1 9 Tf 72 720 Td ({snippet}) Tj ET\n"
        "endstream endobj\n"
        "xref\n0 5\n0000000000 65535 f \n"
        "trailer<</Size 5/Root 1 0 R>>\nstartxref\n0\n%%EOF\n"
    )
